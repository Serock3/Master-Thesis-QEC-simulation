...
transpiled_circ = transpile(circ, coupling_map=[[0,1],[1,2]],optimization_level=0)

display(circ.draw())
display(transpiled_circ.draw())

results = execute(circ,Aer.get_backend('qasm_simulator')).result()
statevector = results.data()['snapshots']['statevector']['snap1'][0]
density_matrix = results.data()['snapshots']['density_matrix']['snap1'][0]['value']

results = execute(transpiled_circ,Aer.get_backend('qasm_simulator')).result()
statevector_transpiled = results.data()['snapshots']['statevector']['snap1'][0]
density_matrix_transpiled = results.data()['snapshots']['density_matrix']['snap1'][0]['value']

print('Fidelity between transpiled and normal statevector snapshots',state_fidelity(statevector_transpiled,statevector))
print('Fidelity between transpiled and normal density matrix snapshots',state_fidelity(density_matrix_transpiled,density_matrix))
print('Counts normal: ',results.get_counts(circ))
print('Counts transpiled: ',results.get_counts(transpiled_circ))

#%% Routing and layout permutation
circ = QuantumCircuit(3)
circ.x(0)
circ.cnot(0,1)
circ.snapshot('snap1', snapshot_type='statevector')
...
display(transpiled_circ.draw(output='mpl'))
