...
pars_hex, cov_hex = scipy.optimize.curve_fit(monoExp, t_hex[1:], fid_L_hex[1:], p0)
pars_DD, cov_DD = scipy.optimize.curve_fit(monoExp, t_DD[1:], fid_L_DD[1:], p0)
pars_pipe, cov_pipe = scipy.optimize.curve_fit(monoExp, t_pipe[1:], fid_L_pipe[1:], p0)
pars_hex_d, cov_hex_d = scipy.optimize.curve_fit(monoExp, t_hex_d[1:], fid_L_hex_d[1:], p0)
pars_DD_d, cov_DD_d = scipy.optimize.curve_fit(monoExp, t_DD_d[1:], fid_L_DD_d[1:], p0)
pars_pipe_d, cov_pipe_d = scipy.optimize.curve_fit(monoExp, t_pipe_d[1:], fid_L_pipe_d[1:], p0)
...
ax.plot(t, monoExp(t, *pars_hex_d)*np.mean(P_L_hex_d[1:]), ':', color='C0',zorder=-5)
ax.plot(t, monoExp(t, *pars_DD_d)*np.mean(P_L_DD_d[1:]), ':', color='C1',zorder=-5)
ax.plot(t, monoExp(t, *pars_pipe_d)*np.mean(P_L_pipe_d[1:]), ':', color='C2',zorder=-5)
