...
if kwargs['conditional']:
    ...
    for key in results.data()[label]:
        for i in range(overlap_cycle):
            ...
            key_int = extract_syndrome(int(key,16),overlap_cycle,num_stab_gens)
            key_up_to_cycle = int(key,16) % (2**(num_stab_gens*(overlap_cycle))*2**4)
            ...
            total_fid_best_single_qb += fid_best_single_qb*counts[key_up_to_cycle]
            total_fid_best_unitary += fid_best_arbitrary_gate*counts[key_up_to_cycle]
    ...
    ax.set_xticklabels([bin(key)[2:].zfill(4) for key in range(16)]+[bin(key)[2:].zfill(4) for key in range(16)]+['max']+['occup.'])
