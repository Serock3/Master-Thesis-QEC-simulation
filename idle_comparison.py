...
def project_dm_to_logical_subspace_V2(rho):
    ...
    logical_pauli_matrices = np.array((
        np.outer(logical[0], logical[0])+np.outer(logical[1], logical[1]),
        Pauli('XXXXX').to_matrix(),
        Pauli('YYYYY').to_matrix(),
        Pauli('ZZZZZ').to_matrix()
    ))
...
res_plus = get_idle_single_qubit(times, snapshot_type=[
                                 'exp', 'dm'], pauliop='X', theta=np.pi/2, phi=np.pi/2, T1=T1, T2=T2)
